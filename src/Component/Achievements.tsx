...
const myAchievementsHtml = myAchievements.map((section, index) => (
    <div key={index} className="flex gap-2 mt-8 ">
        ...
        <p className=" w-[80%] "><span className="text-[#F0F4FA] text-opacity-80 font-bold">{section.title}</span><span className=" text-[#F0F4FA] text-opacity-60">{section.description}</span></p>
        ...
        <div className="bg-cover bg-lightgray bg-[url('/AchievementsImgs/AchievementsBg.png')] py-10 flex flex-col items-center">
            ...
            <div className="flex w-[90%] justify-between">
                <div>
                    ...
                    <p className="text-[#FFFFFF] text-opacity-60 mt-8 w-[80%]">تميز علي النونو بإنجازات متميزة وحصاد جوائز مُشرِّفة خلال مسيرته الكروية المبهرة. من بين أبرز إنجازاته:</p>
                    {myAchievementsHtml}
                </div>

                <img src="/AchievementsImgs/AchievementsMainImg.webp" alt="AchievementsMainImg" className="w-[40%] h-[900px] rounded-xl" />
            </div>
