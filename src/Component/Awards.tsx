...
const awardsDataHtml = awardsData.map((section, index) => (
    <div key={index} className="flex justify-between mb-6">
        ...
        <div className="bg-[url(/AwardsImgs/AwardsBgSmall.webp)] bg-cover h-[200px] w-[30%] border-[1px] border-[#fff] flex justify-around items-center p-4">
            <img src={section.imgUrl} alt={`award ${index} img`} />
            <h1 className="text-[#fff] font-bold text-3xl text-center">{section.title}</h1>
        </div>

        <div className="bg-[url(/AwardsImgs/AwardsBgBig.webp)] bg-cover  h-[200px] w-[68%] border-[1px] border-[#fff] flex justify-center items-center">
